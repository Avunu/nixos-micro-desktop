...
in
{
  config = {
    ...
    # Fcitx5 as a clipboard-history (Super+V) + unicode/emoji picker — NOT
    # for CJK text input. The only input method is the built-in keyboard-us
    # group; every IM-switching hotkey is disabled in globalOptions so fcitx
    # doesn't grab Ctrl+Space / Super+Space. The pickers commit at the text
    # caret over the Wayland input-method protocol (waylandFrontend), so
    # their triggers must reach the app — the compositor must not bind them.
    #
    # Super+V is unbound by niri (see configs/niri/base.kdl) and by umbriel
    # (not in its built-in keymap), so it works unchanged everywhere.
    # Super+. can't be left similarly unbound under umbriel: `Mod+Period` is
    # one of umbriel's hardcoded default keybinds (window-consume-right),
    # with no config-level way to unbind a built-in — any chord assignment,
    # ours or umbriel's own, still consumes the keypress at the compositor
    # level rather than forwarding it. So the unicode picker moves to
    # Super+; under noctalia/umbriel (Win+; is Windows' own alternate emoji
    # shortcut, and is not present anywhere in umbriel's default keymap).
    #
        ...
        fcitx5 = {
          settings = {
            addons = {
              ...
              # Searches symbols/emoji by Unicode name (Win+. style). See the
              # comment above for why noctalia/umbriel uses a different key.
              unicode = {
                globalSection = { };
                sections = {
                  TriggerKey = {
                    "0" = if cfg.desktopShell == "noctalia" then "Super+semicolon" else "Super+period";
                  };
                };
              };
